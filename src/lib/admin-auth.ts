...
export type AdminEnv = { ADMIN_USER?: string; ADMIN_PASSWORD?: string; ADMIN_SESSION_SECRET?: string };
...
export async function verifyAdminCredentials(env: AdminEnv, user: string, password: string): Promise<boolean> {
  ...
  const [expectedUser, providedUser, expectedPassword, providedPassword] = await Promise.all([sha256(env.ADMIN_USER), sha256(user), sha256(env.ADMIN_PASSWORD), sha256(password)]);
...
export async function verifyAdminSession(env: AdminEnv, token: string | null): Promise<AdminSession | null> {
  ...
  if (!payload || !signature || (await hmac(env.ADMIN_SESSION_SECRET, payload)) !== signature) return null;
  try {
    const session = JSON.parse(new TextDecoder().decode(fromBase64Url(payload))) as AdminSession;
    return session?.user && typeof session.exp === 'number' && session.exp > Math.floor(Date.now() / 1000) ? session : null;
  } catch {
    return null;
  }
}
...
export function csrfOk(request: Request): boolean { return request.headers.get('X-Admin-Request') === '1'; }
